...
serve(async (req: Request) => {
  ...
  try {
    ...
    // O evento 'PURCHASE_APPROVED' ou 'PURCHASE_COMPLETE' está no nível superior
    if (hotmartPayload.event === 'PURCHASE_APPROVED' || hotmartPayload.event === 'PURCHASE_COMPLETE') {
      ...
      let purchase_date_timestamp: number; // Agora é um timestamp numérico
      let product_value: number | null = null;

      try {
        // Acessando os dados aninhados sob 'data'
        ...
        purchase_date_timestamp = hotmartPayload.data?.purchase?.approved_date; // Acessando o timestamp
      ...
      if (!transaction_id || !product_name || !buyer_name || !buyer_email || !purchase_date_timestamp) {
        console.error('Payload da Hotmart incompleto: dados essenciais ausentes.', { transaction_id, product_name, buyer_name, buyer_email, purchase_date_timestamp });
        return new Response(
      ...
      // Insere os dados da compra na tabela 'sales', incluindo o payload completo
      const { data, error } = await supabaseAdmin
        ...
        .insert({
          ...
          raw_payload: hotmartPayload, // Salva o JSON completo aqui
        })
        .select();
      ...
      if (error) {
        ...
        throw new Error(error.message); // Re-lança para ser pego pelo catch externo
      }

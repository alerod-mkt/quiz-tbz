...
const QuizScreen: React.FC<QuizScreenProps> = ({ questions, currentQuestionIndex, onAnswer, trackQuestionView }) => {
  ...
  // Novo estado para controlar se o mouse foi movido desde o carregamento da pergunta
  ...
  // Efeito para resetar o estado do mouse e adicionar o listener quando a pergunta muda
  useEffect(() => {
    setHasMouseMoved(false); // Resetar ao carregar uma nova pergunta

    const handleMouseMove = () => {
      setHasMouseMoved(true);
      // Remover o listener assim que o mouse for movido para evitar re-renderizações desnecessárias
      window.removeEventListener('mousemove', handleMouseMove);
    };

    window.addEventListener('mousemove', handleMouseMove);

    // Função de limpeza para remover o listener quando o componente desmonta ou a pergunta muda
    return () => {
      window.removeEventListener('mousemove', handleMouseMove);
    };
  }, [currentQuestionIndex]); // Dependência para re-executar o efeito quando a pergunta muda

  // Classes base e de hover para os botões, condicionadas pelo estado hasMouseMoved
